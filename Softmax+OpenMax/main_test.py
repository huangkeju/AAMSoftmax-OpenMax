...
def test(trainloader, testloader):
    ...
    _, mavs, dists = compute_train_score_and_mavs_and_dists(class_num, trainloader, device, my_net)
    ...
    mavs_t = torch.tensor(np.squeeze(mavs)).to(device)
    with torch.no_grad():
        for inputs, targets in testloader:
            inputs, targets = inputs.to(device), targets.to(device)
            outputs = my_net(inputs)
            dist = torch.sqrt(torch.sum(torch.pow(outputs.view(outputs.size(0), 1, outputs.size(1))-mavs_t, 2), dim=-1))
            outputs0 = F.softmax(outputs, dim=1)
            scores.append(outputs)
            distances.append(dist)
            cscores.append(outputs0)
            labels.append(targets)

    scores = torch.cat(scores,dim=0).cpu().numpy()
    distances = torch.cat(distances,dim=0).cpu().numpy()
